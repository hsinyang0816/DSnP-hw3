...
//	  cout << "Dofilestack size : " << _dofileStack.size() << endl;
   if(_dofile->fail()){
	 delete _dofile;  
...
{
   // TODO...
   CmdMap::const_iterator iterator;
   for (iterator = _cmdMap.begin();iterator != _cmdMap.end();iterator++)
      iterator -> second -> help();
   cout << endl;
}
...
CmdParser::parseCmd(string& option)
{
   ...
   size_t n;
   string origin;
   n = myStrGetTok(str,origin);
   if(n < str.length())
     option = str.substr(n);
   CmdExec* exec = cmdMgr -> getCmd(origin);
   if(exec == NULL)
	 cerr << "Illegal command!! (" << origin << ")" << endl;
   return exec;
}
...
CmdParser::listCmd(const string& str)
{
   ...
   _tabPressCount--;	
   int count = 0;
   int begin = (int)str.find_first_not_of(' ');
   if( begin == -1){
	  cout << endl; 
   ...
   string token = str.substr(begin);
   vector <string> include_cmd;
   for (CmdMap::iterator iter = _cmdMap.begin(); iter != _cmdMap.end(); ++iter){
	  string cmd = iter -> first + iter -> second -> getOptCmd();
	  if(cmd.length() >= token.length())
         if((myStrNCmp(cmd,token,token.length()) == 0) )
            include_cmd.push_back(cmd);
   }
   if((int)include_cmd.size() == 1){
     string element = include_cmd[0].substr(token.length()); 
     for(int i = 0; i < (int)element.length() ; ++i)
	   insertChar(element[i]);
	 insertChar(' ');
	 return;
   }
   else if ((int)include_cmd.size() > 1){
	 cout << endl;  
     for(int i = 0;i < (int)include_cmd.size(); i++){
        cout << setw(12) << left << include_cmd[i];
        if((i+1) % 5 == 0 && i != (int)include_cmd.size() - 1)
		  cout << endl;
	 }
	 reprintCmd();
	 return;
   ...
   _tabPressCount ++;
   int sec_space = (int) token.find_first_of(' ') ;
   string _cmd;
   CmdExec* e;
   if(sec_space > 0) { 
	  _cmd = token.substr(0,sec_space);
	  e = getCmd(_cmd);
	  if(e != 0 ){
		if(_tabPressCount == 1){  
	       cout << endl;
		   ...
		   if(str[(int)str.length()-1] == ' '){
			  ...
			  int c = 0;
              for (int i = 0;;i++){
			     for(int j = 0;j < (int)files.size() - 1;j++){
			 	   if((char)files[j][i] != (char)files[j+1][i]){
                      same = files[j].substr(0,i);
					  c++;
				      break;
				   }
			     }					
			  ...
			  if(same == ""){
                 ...
                 for(int i = 0;i < (int)same.length();++i)
				   insertChar(same[i]);
				 mybeep();
				 return;
			  }
		      ...
		      int last = (int)str.find_last_of(' ') + 1;
		      string last_cmd = str.substr(last);
              listDir(files,last_cmd,".");
			  if((int)files.size() == 1){
                 string prin = files[0].substr((int)last_cmd.length());
				 for(int i = 0;i < (int)prin.length(); ++i)
                    insertChar(prin[i]);
                 insertChar(' ');
				 mybeep();
				 return;
			  }
              else if((int)files.size() > 1){
				 string same;
				 int c = 0;
                 for (int i = 0;;i++){
				    for(int j = 0;j < (int)files.size() - 1;j++){
				 ...
				 if(same == last_cmd){
			       ...
			       return;	
                    ...
                    string prin_not_all = same.substr((int)last_cmd.length());
				    for(int i = 0;i < (int)prin_not_all.length(); ++i)
                       insertChar(prin_not_all[i]);
					mybeep();
				    return;
				 }
			  }
...
CmdParser::getCmd(string cmd)
{
   ...
   for(i = _cmdMap.begin();i != _cmdMap.end();++i){
	 if(myStrNCmp(i->first + i->second->getOptCmd() , cmd , i -> first.length()) == 0){
		e = i->second; 
	   return e;
	 }
   }
